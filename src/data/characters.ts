...
const mmx: CharacterDef = {
  ...
  skill: {
    ...
    damage: [
      { type: DamageType.Physical, atkRatio: 1.3 },
      { type: DamageType.Pure, selfDefMultiplier: 1.5 },
    ],
  },
};
...
const hqs: CharacterDef = {
  ...
  skill: {
    ...
    damage: [
      { type: DamageType.Pure, defMultiplier: 5 },
...
const bmxz: CharacterDef = {
  ...
  growth: { hp: 54, attack: 6, physicalDef: 1.8, magicalDef: 1.2 },
  talent: "友方全场法师每次施加异常→自身技能减1.5s冷却",
  skill: {
    id: "bmxz_charge",
    name: "蓄力",
    tags: [],
    cooldown: 10,
    castTime: 0,
    description: "蓄力1/2/3s（随机）→前排/前中排/全场：攻击×210%魔法",
    damage: [{ type: DamageType.Magical, atkRatio: 2.1 }],
    interruptOnAttack: true,
    aoe: { maxTargets: 9 },
  },
...
const syz: CharacterDef = {
  ...
  talent:
    "攻击时80%概率治疗最低血量友方×40%。阵亡时治疗全法师5s(攻×80%/s,+10%/s)",
  skill: {
    id: "syz_bless",
    name: "神之庇佑",
    tags: [],
    cooldown: 12,
    castTime: 0,
    description: "3名最低血量友方：减伤50%+延迟6s结算+驱散，持续7s",
    effects: [{ type: StatusType.Inspire, duration: 7, value: 0.5 }],
    scriptId: "syz_bless",
  },
};
...
const dyq: CharacterDef = {
  ...
  skill: {
    ...
    cooldown: 7,
    castTime: 0,
    description: "竖排全体攻击×150%魔法+30%灼烧4s（命中3人-30%）",
    damage: [{ type: DamageType.Magical, atkRatio: 1.5 }],
...
const hbfl: CharacterDef = {
  ...
  skill: {
    ...
    cooldown: 12,
    castTime: 1,
    description: "冰封<30%HP龙族4s（免疫物理），每秒攻击×45%+最大HP×4%",
    heal: { atkRatio: 0.45, targetMaxHpRatio: 0.04 },
  },
};
...
const zs: CharacterDef = {
  ...
  skill: {
    ...
    damage: [
      { type: DamageType.Magical, atkRatio: 1.65, targetCurrentHpRatio: 0.1 },
    ],
    aoe: { maxTargets: 9 },
...
const wlgyz: CharacterDef = {
  ...
  skill: {
    ...
    cooldown: 11,
    castTime: 2,
    description: "恢复随机2名友方攻击×180%生命+诅咒随机1名敌方3s",
    heal: { atkRatio: 1.8 },
    aoe: { maxTargets: 2 },
    effects: [{ type: StatusType.Curse, duration: 3 }],
    interruptOnAttack: true,
...
const aysl: CharacterDef = {
  ...
  talent: "对<50%HP敌人附加已损失HP×6%纯粹(≤自身攻击×100%)+禁疗4s",
  skill: {
    id: "aysl_strike",
    name: "暗影突袭",
    tags: ["切入"],
